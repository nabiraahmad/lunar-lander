enum type_of_entity {PLAYER, BADPLATFORM, GOODPLATFORM, WALLS};
class Entity
{
private:
...
public:
    ...
    void move_left()  { m_acceleration.x = -0.02f; };
    void move_right() { m_acceleration.x = 0.02f;  };
    void move_up()    { m_acceleration.y = 0.02f;  };
    ...
    // set acc x
    void const set_acceleration_x(float new_x){
        m_acceleration.x = new_x;
    }
    
    // set acc y
    void const set_acceleration_y(float new_y){
        m_acceleration.y = new_y;
    }
